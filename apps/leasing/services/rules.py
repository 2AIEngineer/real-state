...
class MemberInput:
    ...
    def extra_fields(self) -> dict:
        ...
        return {name: value for name, value in self.extras.items() if name in MEMBER_EXTRA_FIELDS}


def check_member_dates(lease: Lease, joined_at: dt.date) -> None:
    if joined_at < lease.start_date:
        raise InvalidInput("A member cannot join before the lease starts.", field="joined_at")
    if lease.end_date and joined_at > lease.end_date:
        raise InvalidInput("A member cannot join after the lease ends.", field="joined_at")
...
def check_member_account(user) -> None:
    ...
    if not user.is_active or user.is_technical_account:
        raise InvalidInput("Lease members must have an active personal account.", field="user_id")
