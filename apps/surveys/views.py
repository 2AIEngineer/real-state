...
class SurveyListView(_AnsweredSurveys, BaseAPIView):
    ...
    def post(self, request):
        ...
        return self.render(
            s.SurveySerializer,
            SurveyService.get_visible(actor=request.user, survey_id=survey.pk),
...
class SurveyDetailView(_SurveyView):
    ...
    def get(self, request, survey_id: int):
        return self.render(
            s.SurveySerializer, SurveyService.get_visible(actor=request.user, survey_id=survey_id)
        )

    @extend_schema(request=s.SurveyUpdateSerializer, responses=s.SurveySerializer)
    def patch(self, request, survey_id: int):
        survey = SurveyService.get_visible(actor=request.user, survey_id=survey_id)
