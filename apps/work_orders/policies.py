...
class WorkOrderPolicy:
    ...
    def visible_filter(user) -> Q:
        return Q(property_id__in=AccessService.managed_property_ids(user)) | Q(assignee=user)

    @staticmethod
    def can_view(user, wo: WorkOrder) -> bool:
        return wo.assignee_id == user.pk or AccessService.manages_property(user, wo.property)
